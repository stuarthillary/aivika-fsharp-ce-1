...
type TableProvider =
    ...
    /// Gets the text used when rendering a link to the CSV file for multiple runs (supports variables $RUN_INDEX, $RUN_COUNT and $LINK).
    member RunLinkText: string

    /// Sets the text used when rendering a link to the CSV file for multiple runs (supports variables $RUN_INDEX, $RUN_COUNT and $LINK).
    member RunLinkText: string with set

    /// Specifies the CSV file name for each run (supports variables $RUN_INDEX, $RUN_COUNT and $TITLE).
